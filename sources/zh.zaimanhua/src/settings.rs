use aidoku::{
	...
	imports::defaults::{DefaultValue, defaults_get, defaults_set, defaults_set_data},
};
...
pub fn get_proxy_url() -> Option<String> {
	defaults_get::<String>(PROXY_URL_KEY)
		.filter(|url| url.starts_with("https://") && url.len() > 10 && !url.contains("your-worker"))
...
pub fn get_user_cache() -> Option<UserCache> {
	defaults_get::<UserCache>(USER_CACHE_KEY)
}
...
pub fn set_user_cache(level: i32, is_sign: bool) {
	...
	defaults_set_data(USER_CACHE_KEY, &cache);
}
